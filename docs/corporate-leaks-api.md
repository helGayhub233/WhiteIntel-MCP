...
> Source: https://docs.whiteintel.io/whiteintel-api-doc/leak-discovery/corporate-leaks-api
> date: 2026-05-10
...
```
POST https://api.whiteintel.io/get_corporate_leaks.php
```
...
```bash
curl -X POST https://api.whiteintel.io/get_corporate_leaks.php \
...
- **Pagination:** For organizations with large exposure footprints, paginate using `page` and `limit` rather than requesting a single oversized result set. The maximum permitted `limit` is `5000`.
