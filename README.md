...
MCP Host 完成认证；`WHITEINTEL_MCP_ALLOW_INSECURE_REMOTE=true` 仅用于已有可信
认证代理的场景。嵌入部署时，`create_server()` 支持 MCP SDK 的 `AuthSettings` 和
`TokenVerifier`。

SDK 2.0.0 已移除 `mount_path`。需要挂载到现有 ASGI 应用时，请使用
`streamable_http_app()` 并由宿主应用管理 lifespan。
...
（wire JSON 中为 `isError: true`，Python SDK 字段为 `is_error=True`），并保留上游消息、HTTP 状态和限流信息。稳定错误码包括
...
```bash
PYTHONPYCACHEPREFIX=/tmp/whiteintel-pyc python -m compileall src/whiteintel_mcp
uv build --wheel
```

项目同时兼容 MCP `2026-07-28` 和 `2025-11-25`；版本记录见 `CHANGELOG.md`。
